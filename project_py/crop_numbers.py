    ...
    for i in range(9):
        for j in range(9):
            ...
            if max_area >100:
                ...
                cv2.imwrite("./cell_data/%d%d.png" % (j, i), img2)

    print("저장완료")

def image_process(img, is_debug):
    ...
    if Debug:
        cv2.imshow('After Normalization', res2)
        cv2.waitKey()
        cv2.destroyAllWindows()
    ...
    #CV_ADAPTIVE_THRESH_GAUSSIAN_C 는 가중치가 가우시안 형태로 지정되며, 중심 쪽 픽셀 값의 가중치가 높게 부여된다.
    ...
    ret, dx_norm = cv2.threshold(dx, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    close = cv2.morphologyEx(dx_norm, cv2.MORPH_DILATE, kernelx, iterations = 1)
    if Debug:
        cv2.imshow("Edge Normalize", dx_norm)
        cv2.imshow("Close", close)


    contour, hr = cv2.findContours(close, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE) # Find Contours로 x방향 수직 선들을 찾음
    for cnt in contour:
    ...
    cnt, hier = cv2.findContours(close, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE) # Find Contours로 y방향 수직 선들을 찾음
    for i in range(len(cnt)):
    ...
    cell = devide_cell(output) ############# 숫자 있는 부분만 짤라서 저장
    return points_cpy
